...
public:
    Ghost(std::vector<Component*> components) :
        Entity(components),
        maxSpeed(20),
    ...
    void move(Player* player){
        float currX = dynamic_cast<Physics*>(player->getComponent(phys))->getX();
        float currY = dynamic_cast<Physics*>(player->getComponent(phys))->getY();
        float deltaX;
        float deltaY;
        if(!isAttacking){
            deltaX = x - currX;
            ...
            dynamic_cast<Physics*>(getComponent(phys))->applyForce(vec);
        }
    }
